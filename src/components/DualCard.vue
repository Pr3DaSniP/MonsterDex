<template>
    <div
        ...
        <div class="relative aspect-square w-full">
            <img :alt="variantA.name" :src="API_IMG_URL + variantA.image_filename"
                :class="['absolute top-0 left-0 w-full h-full object-cover clip-left card-image', !monster.owned ? 'blackandwhite' : '']"
                loading="lazy" />
            <img :alt="variantB.name" :src="API_IMG_URL + variantB.image_filename"
                :class="['absolute top-0 left-0 w-full h-full object-cover clip-right card-image', !monster.owned ? 'blackandwhite' : '']"
                loading="lazy" />
            <Typeicon :size="32" :type="monster.element"></Typeicon>
            <div class="absolute bottom-4 right-2 flex items-end flex-col gap-2">
                <Checkbox label="Possédé" v-model="monster.owned"></Checkbox>
                <Checkbox label="Full Skill" v-model="monster.full_skill"></Checkbox>
            </div>
        </div>
...
const variantA = computed(() => monster.variants[0])
const variantB = computed(() => monster.variants[1])

const name = computed(() => {
  return truncate(variantA.value.name, 15) + ' | ' + truncate(variantB.value.name, 15)
})

const isHidden = computed(() => {
  if (!props.filter) return false
  const query = props.filter.toLowerCase()
  return !monster.variants.some(v => v.name.toLowerCase().includes(query))
})
